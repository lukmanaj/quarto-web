...
In particular, the table cell HTML attributes `colspan` and `rowspan` are supported by all formats, but can't be expressed using  Quarto Markdown. (??? - need confirmation, think @cscheid said there was one way but it was awkward, does not seem to be documented) 

Markdown supports specification of alignment by column, but you can set table cell CSS properties `text-align` and `vertical-align` only through Raw HTML. These are not currently supported by PDF/Latex or Pptx output formats, and Docx only supports `text-align`.
