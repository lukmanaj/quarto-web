...
### Features available only in HTML Tables

Some table features are difficult or impossible to specify using Markdown tables.

In particular, cells spanning multiple rows or columns are supported by most output formats, but it is tricky to draw grid tables with spans, and there is no way to express them in pipe tables. If you are comfortable with HTML or use a package that generates HTML, the table cell HTML attributes `colspan` and `rowspan` are probably the best option.

Markdown supports specification of alignment by column, but does not allow setting the alignment of individual cells. The CSS properties `text-align` and `vertical-align` are only through Raw HTML. (These are not currently supported by PDF/Latex or Pptx output formats, and Docx only supports `text-align`.)
...
These features are available in PDF, Typst, and Docx in addition to HTML.
